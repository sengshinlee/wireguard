...
<h1>WireGuard</h1>
<ul>
<li><a href="https://wireguard.sengshinlee.com/mirrors/wireguard-ubuntu.sh">wireguard-ubuntu.sh</a></li>
...
WG_WINDOWS_URL="https://download.wireguard.com/windows-client/"

WG_WINDOWS_AMD64=$(curl -s ${WG_WINDOWS_URL} | cut -d '"' -f 4)
wget -t 0 ${WG_WINDOWS_URL}${WG_WINDOWS_AMD64} -O wireguard-windows-latest-amd64.msi
WG_WINDOWS_ARM64=$(curl -s ${WG_WINDOWS_URL} | cut -d '"' -f 6)
wget -t 0 ${WG_WINDOWS_URL}${WG_WINDOWS_ARM64} -O wireguard-windows-latest-arm64.msi

WG_ANDROID_URL=$(curl -s https://f-droid.org/en/packages/com.wireguard.android/ | \
                grep -E '_[0-9]+.apk"' | \
                awk -F '"' '{print $2}' | \
                head -1)
wget -t 0 ${WG_ANDROID_URL} -O wireguard-android-latest.apk

cat >wireguard-ubuntu.sh <<WG_UBUNTU_EO'F'
#!/bin/bash
...
function down_wg() {
    local COUNT=1
    local SERVER_WG_SHOW_NIC_NUM

    if [ ${IS_INSTALLED} -eq 0 ]; then
        while [ ${COUNT} -le 256 ]; do
            local SERVER_WG_SHOW_NIC_NUM=$(wg | grep "interface" | awk '{print $2}' | cut -c 3- | head -${COUNT} | tail -1)
            if [ -z "${SERVER_WG_SHOW_NIC_NUM}" ]; then
                if [ ! -f "/etc/wireguard/wg${SERVER_WG_SHOW_NIC_NUM}.conf" ]; then
                    touch /etc/wireguard/wg${SERVER_WG_SHOW_NIC_NUM}.conf
                fi
            fi
            COUNT=$(expr ${COUNT} + 1)
        done
...
function uninstall_wg_tools() {
    if [ ${IS_INSTALLED} -eq 0 ]; then
        local SERVER_WG_SHOW_NIC=$(wg | grep "interface" | awk '{print $2}' | head -1)
        while [ "${SERVER_WG_SHOW_NIC}" != "" ]; do
            if [ ! -f "/etc/wireguard/${SERVER_WG_SHOW_NIC}.conf" ]; then
                touch /etc/wireguard/${SERVER_WG_SHOW_NIC}.conf
            fi

            if [ ! -f "/etc/wireguard/${SERVER_WG_SHOW_NIC}-client.conf" ]; then
                touch /etc/wireguard/${SERVER_WG_SHOW_NIC}-client.conf
            fi
            wg-quick down ${SERVER_WG_SHOW_NIC} >/dev/null 2>&1
            SERVER_WG_SHOW_NIC=$(wg | grep "interface" | awk '{print $2}' | head -1)
        done
...
function help() {
    echo '
USAGE
  bash wireguard-ubuntu.sh [OPTION]
...
OPTION
  ...
  -s, --show    Shows the current active interface
