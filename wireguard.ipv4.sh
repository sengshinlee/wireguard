...
function generate_wg_ifs() {
    ...
    if [ ${WG_TOOLS_STATUS} -eq 0 ]; then
        ...
        local CLIENT_WG_IF_CONF="${SERVER_WG_IF_CONF_ROOT_DIR}/${SERVER_WG_NIC}-client.ipv4.conf"
        local CLIENT_WG_SPLIT_TUNNELING_IF_CONF="${SERVER_WG_IF_CONF_ROOT_DIR}/${SERVER_WG_NIC}-client.ipv4.split-tunneling.conf"
...
MTU = ${WG_MTU}
PreUp = sysctl -w net.ipv6.conf.all.disable_ipv6=1 >/dev/null 2>&1
PreUp = sysctl -w net.ipv6.conf.default.disable_ipv6=1 >/dev/null 2>&1
PreUp = sysctl -w net.ipv6.conf.lo.disable_ipv6=1 >/dev/null 2>&1
...
PostDown = sysctl -w net.ipv6.conf.all.disable_ipv6=0 >/dev/null 2>&1
PostDown = sysctl -w net.ipv6.conf.default.disable_ipv6=0 >/dev/null 2>&1
PostDown = sysctl -w net.ipv6.conf.lo.disable_ipv6=0 >/dev/null 2>&1
...
function generate_all_clients_wg_ifs() {
    ...
    if [ -f "wg0.conf" ] && [ -f "wg0-client.ipv4.conf" ] && [ -f "wg0-client.ipv4.split-tunneling.conf" ]; then
        local CLIENT_WG_IF_CONFS=(
            "3 asuswrt.ipv4.conf"
            "4 ubuntu.ipv4.conf"
            "5 macos.ipv4.conf"
            "6 windows.ipv4.conf"
            "6 windows.ipv4.split-tunneling.conf"
            "7 ios.ipv4.conf"
            "8 ipados.ipv4.conf"
            "9 android.ipv4.conf"
        )
        ...
        for A_CLIENT_WG_IF_CONF in "${CLIENT_WG_IF_CONFS[@]}"; do
            ...
            if [[ "${FILENAME}" == *"split-tunneling"* ]]; then
                cp wg0-client.ipv4.split-tunneling.conf "${FILENAME}"
                ...
                cp wg0-client.ipv4.conf "${FILENAME}"
                ...
                sed -i "s|^\(Address\s*=\s*\).*|\1${CLIENT_WG_IPV4}/32|" "${FILENAME}"
...
EOF
        ...
        echo "  - /etc/wireguard/wg0-client.ipv4.conf"
        echo "  - /etc/wireguard/wg0-client.ipv4.split-tunneling.conf"
        exit 1
    fi
}
